...
| F2 | Shared browser leases on adaptive URL writing, analysis/planning/Flow and thumbnails; encoder lease on both renderer entrypoints. Adaptive Audacity and stitching now share the Audacity lease, and adaptive Audacity refuses to seize unrelated open processes. | TTS/browser/clipboard ownership and legacy processes do not yet share all locks. |
