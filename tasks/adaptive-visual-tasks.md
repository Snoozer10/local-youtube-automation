...
- PR #21 CI follow-up: the Windows runner's FFmpeg 9 rejected deprecated `-filter_complex_script` in the real render test. Both render paths now use `-/filter_complex`, with the same file-backed graph and a passing local real-render regression. Await a green rerun before treating the PR as merge-ready.
