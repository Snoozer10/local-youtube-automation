...
The shot planner sends at most about 20 seconds of canonical narration per Gemini request, retaining original span IDs and binding every validated partial batch to the brief, timeline and window boundaries in `shot_plan.partial.json`. A later transport failure can resume after that batch. Changed upstream or planner recipes archive the checkpoint with other mutable activations. A technically valid batch still needs editorial review; Professor Yashrah's archived gears/puzzle shorthand is an explicit reject. The current local compositor cannot render the precise 6×6 Schulte grid or timer, so that generic graphic primitive remains a prerequisite for the Professor preview.
