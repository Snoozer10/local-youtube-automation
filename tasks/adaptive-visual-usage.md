...
An edit first reopens the receipt's exact Flow project and attaches the matching provider image, tolerating signed-URL refresh only when the stable provider media ID matches. Chip counts include only visible ingredients inside the active prompt composer. If that project reloads without its provider cards, the engine re-uploads the receipt-bound content-addressed accepted PNG and verifies exactly one prompt ingredient by provider ID or pixel digest. The same exact check runs again immediately before prompt submission; a standalone shot requires zero attached references. It never chooses a recent or merely similar card. Missing, stale or mismatched references block generation; restoration into a different account remains unsupported.
