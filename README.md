...
| :---- | :------------------ | :--------------- | :------------ | :----------------- | :----------------- |
| **01** | `automate_all.py` | Captions ingestion & conversational transcreation across adaptive channel niches. Enforces $\ge 35\%$ script ratio gate. | `youtube_urls.txt`, `prompts/phase1.txt`, `prompts/phase3.txt` | `raw_transcript.txt`, `breaked_paragraphs.txt`, `final_output.txt`, `.docx` | `checkpoint.json` |
| **02** | `refine_script.py` | Injects 1-3-1 sentence cadence, comedic timing, niche-specific hooks, and phonetic Tashkeel diacritics. | `final_output.txt`, `prompts/refine.txt`, `daheeh_config.json` | `refined_script.txt`, `refined_script.docx`, `tts_payload.json` | `refine_checkpoint.json` |
