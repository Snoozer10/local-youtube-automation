...
def main():
    ...
    egyptian_arabic_prompt = "الجاس لايتنج، الجاسلايتنج، يا عم، بتهلوس، ميكروباص، المريونيط، اللنبة، سدق، بتلعي، الرموت كونترول."
    ...
    try:
        ...
        if not all_words:
            ...
            for i, word_obj in enumerate(all_words):
                ...
                next_gap = (
                    (all_words[i + 1].get("start", word_obj.get("end", 0)) - word_obj.get("end", 0))
                    if not is_last
                    else 0.0
                )
                ...
                if (
                    ...
                    or next_gap > 1.2
                    or len(img_words) >= 18
                    or (len(img_words) >= 10 and (has_punct or next_gap > 0.4))
                ):
        ...
        if "OutOfMemoryError" in str(e) or "CUDA out of memory" in str(e):
            ...
            print(
                "To fix: Open voice_option_notes.txt and change Whisper Model from 'small' to 'base'."
            )
            sys.exit(1)
