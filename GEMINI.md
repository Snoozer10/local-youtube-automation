...
- Adaptive shot plans replace legacy roadmap camera cycles. They reference canonical narration spans with contiguous integer frames; they never rewrite speech timing. Holds are valid, motion is selective, and typography/annotations are rendered locally. Planning requests are bounded by narration time, not just ASR span count, and each validated window is saved in a content-bound partial checkpoint for resume. One Gemini chat is bound to each planning window; validation repair stays in that chat, while transport retry reuses the exact prompt. Atomic prompt-hash receipts preserve submitted, timed-out and completed attempts so an interrupted or late response is recovered from its bound chat before resubmission.
