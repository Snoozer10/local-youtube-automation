...
- Browser (including adaptive TTS), adaptive Audacity and encoder leases fence shared devices and short publication operations. Adaptive CLI stages are durable run-bound SQLite jobs: unchanged successful recipes skip only after output validation, missing/corrupt outputs repair automatically, content-bound existing outputs may be adopted, changed content/config creates new attempt lineage, expired workers are reclaimed with a higher fence, and three consecutive failures open a circuit until explicit `--force-retry`. Stage and distinct device claims nest and jointly fence publication. Adaptive TTS uses an owned tab in an existing CDP session; it does not launch, rotate or terminate the user's browser. Immutable final filenames precede atomic activation pointers; byte-verified clip caches include render/tool recipes. Legacy tools do not all participate.
