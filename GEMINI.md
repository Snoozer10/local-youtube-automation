...
- Accepted asset copies and hashes outlive provider scratch paths. When the receipt-bound Flow project reloads without provider cards, restore only the exact content-addressed accepted PNG and verify one prompt ingredient; never substitute a recent card. Full-frame background OCR must actually run. Technical verification is separate from human editorial approval.
