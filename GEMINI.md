...
- Accepted asset copies and hashes outlive provider scratch paths. Asset publication journals the content-bound receipt intent before copying accepted bytes; restart recovery activates only a complete image matching the current recipe, reference, hash, pixels and geometry. When the receipt-bound Flow project reloads without provider cards, restore only the exact content-addressed accepted PNG and verify one prompt ingredient; never substitute a recent card. Full-frame background OCR must actually run. Technical verification is separate from human editorial approval.
