...
class TestLoadAiCameraDecisions:
    ...
    def test_valid_json_blocks_parsed(self):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            data = [
                {
                    "timestamp": "[00:00]",
                    "visual_prompt": {"camera_specifications": "zoom in slowly"},
                },
                {
                    "timestamp": "[00:08]",
    ...
    def test_missing_visual_prompt_or_camera_spec_defaults_to_static(self):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            data = [
                ...
                {
                    "timestamp": "[00:05]",
                    "visual_prompt": {"camera_specifications": "zoom out"},
                },  # valid
                {"timestamp": "[00:10]"},  # no visual_prompt -> static
                {
                    "timestamp": "[00:15]",
                    "visual_prompt": {"camera_specifications": "static"},
                },  # valid
            ]
            json.dump(data, f)
            f.flush()
    ...
    def test_zero_hour_timestamp_gets_dual_keys(self):
        """[HH:MM:SS] with zero hours maps to both full and MM_SS normalized keys."""
        with tempfile.NamedTemporaryFile(
            mode="w", suffix=".json", delete=False, encoding="utf-8"
        ) as f:
            data = [
                {"timestamp": "[00:01:30]", "visual_prompt": {"camera_specifications": "zoom in"}},
            ]
            json.dump(data, f)
            f.flush()
            result = compile_video.load_ai_camera_decisions(f.name)
