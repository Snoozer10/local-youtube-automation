"""Audio chapter stitcher module.

Provides lossless WAV frame concatenation and format homogeneity validation
across sequential audio chapters.
"""
...
def main():
    ...
    # Prioritize polished_chapters only if it has at least as many chapters as voice_chapters
    polished_count = 0
    if os.path.exists(polished_dir):
        polished_count = len(
            [f for f in os.listdir(polished_dir) if f.startswith("Chapter_") and f.endswith(".wav")]
        )

    voice_count = 0
    if os.path.exists(voice_dir):
        voice_count = len(
            [f for f in os.listdir(voice_dir) if f.startswith("Chapter_") and f.endswith(".wav")]
        )

    if polished_count > 0 and polished_count >= voice_count:
        chapters_source_dir = polished_dir
        print(
            f"[SYSTEM] Found {polished_count} polished chapters (>= {voice_count} raw). "
            "Utilizing 'polished_chapters' as the audio source."
        )
    elif voice_count > 0:
        chapters_source_dir = voice_dir
        print(
            f"[SYSTEM] Utilizing complete raw 'voice_chapters' ({voice_count} chapters vs {polished_count} polished) as the audio source."
        )
    else:
        print(
