"""Transcript spelling corrector module.

Performs fuzzy sequence matching (difflib.SequenceMatcher) against refined script text
to correct transcription errors in timestamped transcript text and SRT files
without disturbing speech alignment timestamps.
"""
...
def correct_transcript():
    ...
    # 0. Authoritative check: if audio_manifest.json exists, reconstruct the exact spoken text
        ...
        try:
            ...
            if segments:
                ...
                if spoken_paras:
                    ...
                    ref_words = full_spoken.split()
                    print(
                        f"[SYSTEM] Found audio manifest. Utilizing spoken text from '{manifest_path}' "
                        f"({len(ref_words)} words across {len(segments)} segments) as authoritative spelling reference.",
                        flush=True,
                    )
        except Exception as e:
            print(f"[SYSTEM] Note: Could not read spoken text from audio_manifest.json: {e}", flush=True)
    ...
    if ref_words is None:
        ...
        if os.path.exists(ref_refined):
            ...
            print(
                f"Error: Neither 'audio_manifest.json', 'refined_script.txt', nor 'final_output.txt' found in '{target_dir}'.",
                flush=True,
            )
            return
