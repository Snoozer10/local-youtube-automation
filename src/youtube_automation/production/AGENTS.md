...
- Accepted image copies and final videos use content-derived names. Preview/master publication journals record the verified pointer before immutable-file activation; a retry may activate an orphan only after its generation, full inputs, path, hash, frame rate, frame count and audio stream revalidate. active_master.json is the only adaptive master activation pointer.
