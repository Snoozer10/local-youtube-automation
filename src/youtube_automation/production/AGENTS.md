...
- Bind one Gemini chat to each planning window. Validation repair remains in that chat; transport retry reuses the exact prompt. Persistent planning defaults to a content-addressed UTF-8 file attachment plus a short hash-bound instruction; invalid transport values fail before submission. Save atomic prompt-hash response receipts with model, transport, attachment, window, stable `/app/<conversation-id>` URL and attempt states. Mark schema-invalid completed responses rejected before repair so restart cannot replay them as success. Persist Ctrl+C as interrupted before re-raising it. On restart, recover stable late responses from submitted, interrupted or timed-out attempts before opening a new chat or resubmitting; never navigate the transient blank `/app` route as a receipt target.
