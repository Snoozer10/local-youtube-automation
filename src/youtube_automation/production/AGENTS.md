...
- Bind one Gemini chat to each planning window. Validation repair remains in that chat; transport retry reuses the exact prompt. Save atomic prompt-hash response receipts with model, window, chat URL and attempt states. On restart, recover stable late responses from submitted or timed-out attempts before opening a new chat or resubmitting.
