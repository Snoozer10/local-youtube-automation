...
def import_source_narration(
    ...
    try:
        ...
        with publication_guard():
            ...
            if _sha256(audio) != source_receipt["audio_sha256"] or any(
                (root / name).read_text(encoding="utf-8") != value
                for name, value in outputs.items()
            ):
                raise ValueError("Published source narration bytes failed verification")
            atomic_write_json(str(root / "adaptive_writing_receipt.json"), writing_receipt)
            # This receipt activates the import, so it must be the final publication write.
